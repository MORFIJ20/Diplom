...
class DevicesListFragment : Fragment(), ItemAdapter.Listener {
    ...
    private fun initRcViews() = with(binding){
        rcViewPained.layoutManager = LinearLayoutManager(requireContext())
        rcViewSearch.layoutManager = LinearLayoutManager(requireContext())
        itemAdapter = ItemAdapter(this@DevicesListFragment)
        discoveryAdapter = ItemAdapter(this@DevicesListFragment)
        rcViewPained.adapter = itemAdapter
    ...
    private val bReceiver = object : BroadcastReceiver(){
        override fun onReceive(p0: Context?, intent: Intent?) {
           ...
           try{
               Log.d("MyLog", "Devices: ${device?.name}")
           } catch (e: SecurityException){}
